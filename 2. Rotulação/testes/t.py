...
def rot_adicionado(valor, rotulos):
    """Checa se a equivalência do rótulo já foi adicionada"""
    for rotulo, equivalentes in rotulos.items():
        for equivalente in equivalentes:
            if equivalente == valor:
                return True
...
def resolve_equivalencias(matriz_rotulos, equivalentes, ultimo_rotulo):
    ...
    for rotulo in range(1, ultimo_rotulo + 1):
        for tupla in equivalentes:
            if tupla[0] == rotulo and not rot_adicionado(tupla[1], rotulos):
                equivalencias.append(tupla[1])
            elif tupla[1] == rotulo and not rot_adicionado(tupla[0], rotulos):
                equivalencias.append(tupla[0])
        
        rotulos[rotulo] = equivalencias
        equivalencias[:] = []
    ...
    for i in range(matriz_rotulos.shape[0]):
        for j in range(matriz_rotulos.shape[1]):
            for rotulo, equivalentes in rotulos.items():
                for equivalente in equivalentes:
                    if equivalente == matriz_rotulos[i, j]:
                        matriz_rotulos[i, j] = rotulo
...
imagem = cv.imread('../imagens/lena128.bmp')
